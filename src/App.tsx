  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/30">
      ...
      <header className="border-b border-gray-100 bg-white/70 backdrop-blur-md sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-2 text-xs text-gray-400">
            ...
            <span className="px-2 py-1 rounded-lg bg-gray-50 font-medium">8 команд</span>
            <span className="px-2 py-1 rounded-lg bg-gray-50 font-medium">Риски и SOP</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        {/* Operations: risks, channels, segment tactics, incident response */}
        <AnimatePresence mode="wait">
          <motion.div
            key={`ops-${currentStage.id}`}
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            transition={{ duration: 0.3 }}
